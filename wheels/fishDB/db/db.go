...
// Open 开启一个数据库实例
func Open(dirPath string) (*FishDB, error) {
	// 如果数据库目录不存在，则新建一个
...
// Put 写入数据
func (db *FishDB) Put(key []byte, value []byte) (err error) {
	if len(key) == 0 {
		return
	}
...
// Get 取出数据
func (db *FishDB) Get(key []byte) (val []byte, err error) {
	if len(key) == 0 {
		return
	}
...
// Del 删除数据
func (db *FishDB) Del(key []byte) (err error) {
	if len(key) == 0 {
		return
	}
...
// 从文件当中加载索引
func (db *FishDB) loadIndexesFromFile() {
	if db.dbFile == nil {
		return
	}
